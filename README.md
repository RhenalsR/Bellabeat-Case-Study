...
The Data files considered were `DailyActivity_merged.csv` and `SleepDay_merged.csv`. Initially `weightLogInfo_merged.csv` was also considered but later on was discarded since it contains data for only 8 users, and I considered this sample too small to give any recommendation based on its analysis.
...
WHERE Calories != 0 -- rows with calories value = 0 are fully blank. Excluded --
ORDER BY
 Id, ActivityDate DESC
```

![Preview of the output table from SQL. Limited to 7 rows](/Base_table.png)
_Table output limited to 7, of 939 rows_
...
## Visualizations
After checking the averages and maximums I found a big difference between the sedentary time and everything else, so I created some Tableau graphs to confirm this preview. 

![Tableau visualization no 1. Bar chart of average time spent](/Avg_time_comp.png)

In this visualization, we can confirm what our table with average showed us: the average sedentary time spent is much bigger than the average active time spent.
With an average sedentary time of 775.64 minutes, or 12.92 hours, more than a whole working day. While the average active time (moderate and very active time) sum is 43.27 minutes, less than an hour, certainly a big difference.

<br />

![Tableau visualization no 1. Bar chart of average time spent](/Daily_comparison2.png)
...
![Tableau visualization no 1. Bar chart of average time spent](/Sleep_comp.png) 

Doing some more profound research a chart comparing the average time slept and average sedentary time, having a better perspective of the amount of inactive time. And is also appreciated the irregularity of the sleeping hours, despite the anomaly, the average has been maintained in a good sleeping time, since the recommendation is to sleep between 6-8 hours(360-480 min).

<br />

![Tableau visualization no 1. Bar chart of average time spent](/Calories_correlation.png)

It's visible how the Steps walked and calories burnt have a positive correlation, and it's also good to see a slight increase in the average Calories burnt thorough the period tested. That could be attributed to the health app track and/or suggestions, suggestions that could've made the users burn a little more calories progressively.
...
![woman doing yoga, purple background](https://medicaldialogues.in/wp-content/uploads/2015/10/Yoga-Purple-Background.jpg
)
